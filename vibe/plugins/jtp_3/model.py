...
def sdpa_attn_mask(
    ...
    if num_prefix_tokens:
        mask = torch.cat(
            (torch.ones(*mask.shape[:-1], num_prefix_tokens, device=patch_valid.device, dtype=torch.bool), mask), dim=-1
        )
...
def patchify_image(
    img: Image.Image, patch_size: int, max_seq_len: int, share_memory: bool = False
...
def load_image(
    path: str, patch_size: int = 16, max_seq_len: int = 1024, share_memory: bool = False
) -> tuple[Tensor, Tensor, Tensor]:
...
def load_model(
    ...
    *,
    ...
    model = NaFlexVit(len(labels), device="cpu", dtype=dtype)

    match arch[31:]:
        case "+rr_hydra":
            from .hydra_pool import HydraPool

            attn_pool = HydraPool.for_state(state_dict, "attn_pool.", device=device, dtype=dtype)

            model.attn_pool = attn_pool
            model.head = attn_pool.create_head()
    ...
    for ext_path in extensions:
        ...
        if ext_metadata["architecture"] != arch:
            raise RuntimeError(
                f"Extension {repr(ext_path)} has incompatible architecture {repr(ext_arch)}, expected {repr(arch)}."
            )

        if (conflict_path := ext_labels.get(ext_label)) is not None:
            raise RuntimeError(
                f"Extension {repr(ext_path)} conflicts with extension {repr(conflict_path)} over label {repr(ext_label)}."
            )
