...
_INTENT_FLAGS = {
    ...
    Intent.RELATIVE_COLORIMETRIC: (image_cms.Flags.HIGHRESPRECALC | image_cms.Flags.BLACKPOINTCOMPENSATION),
...
def process_srgb(
    ...
    *,
    ...
    if expect is not None and size != expect:
        raise RuntimeError(f"Image is {size[0]}x{size[1]}, but expected {expect[0]}x{expect[1]}.")
    ...
    if (icc_raw := img.info.get("icc_profile")) is not None:
        ...
        try:
            ...
            if img.mode == mode:
                profileToProfile(img, profile, _SRGB, renderingIntent=intent, inPlace=True, flags=flags)
            else:
                img = cast(
                    Image, profileToProfile(img, profile, _SRGB, renderingIntent=intent, outputMode=mode, flags=flags)
                )
    ...
    if resize is not None and size != resize:
        img = img.resize(resize, Resampling.LANCZOS, box=crop, reducing_gap=3.0)
        crop = None
...
def put_srgb_patch(img: Image, patch_data: Tensor, patch_coord: Tensor, patch_valid: Tensor, patch_size: int) -> None:
    ...
    coords = np.stack(
        np.meshgrid(
            np.arange(patches.shape[0], dtype=np.int16), np.arange(patches.shape[1], dtype=np.int16), indexing="ij"
        ),
        axis=-1,
    )
