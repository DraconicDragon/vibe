...
from torch.nn import LayerNorm, Linear, Module, ModuleList, Parameter, RMSNorm
from torch.nn.functional import normalize, scaled_dot_product_attention
...
from .glu import GLU, SpGLU, SwiGLU
...
class _FeedForward(Module):
    def __init__(
        ...
        *,
        ...
        self.norm = LayerNorm(input_dim, elementwise_affine=affine, device=device, dtype=dtype)
        self.proj_in = glu(input_dim, hidden_dim, dropout=dropout, device=device, dtype=dtype)
        self.proj_out = Linear(hidden_dim, output_dim, bias=False, device=device, dtype=dtype)
...
class _MidBlock(Module):
    def __init__(
        ...
        *,
        ...
        self.o_proj = Linear(attn_dim, attn_dim, bias=False, device=device, dtype=dtype)
    ...
    def _forward_attn_varlen(
        self, x: Tensor, k: Tensor, v: Tensor, cu_q: Tensor, cu_seq: Tensor, max_q: int, max_seq: int
    ) -> Tensor:
        shape = x.shape
...
class HydraPool(Module):
    def __init__(
        ...
        *,
        ...
        self.q = Parameter(torch.randn(n_heads, n_classes, head_dim, device=device, dtype=dtype))
        ...
        self.mid_blocks = ModuleList(
            _MidBlock(
                n_classes, attn_dim, head_dim, ff_dim, glu=ff_glu, ff_dropout=ff_dropout, device=device, dtype=dtype
            )
    ...
    def _forward_attn_varlen(
        self, x: Tensor, cu_q: Tensor, cu_seq: Tensor, max_seq: int
    ...
    def forward_varlen(self, x: Tensor, cu_seq: Tensor, max_seq: int) -> Tensor:
        cu_q = torch.arange(0, cu_seq.size(0) * self.n_classes, self.n_classes, device=self.q.device, dtype=torch.int32)
