...
from torch.nn import Identity, LayerNorm, Linear, Module, ModuleList, Parameter
from torch.nn.functional import gelu, interpolate, scaled_dot_product_attention
...
class NaFlexEmbeds(Module):
    def __init__(
        ...
        *,
        ...
        self.cache: dict[tuple[int, int], Tensor] | None = None
    ...
    def _apply_pos_embed_padded(
        self, patches: Tensor, sizes: Tensor, compute_valid: bool
    ...
    @staticmethod
    def _unpad(
        patches: Tensor, sizes: Tensor, valid: Tensor | None
    ) -> Iterable[tuple[Tensor, tuple[int, int], Tensor | None]]:
        if patches.ndim == 2:
    ...
    def _get_pos_embed(self, shape: tuple[int, int]) -> Tensor:
        ...
        if shape == (16, 16):
            return rearrange(self.pos_embed, "1 h w c -> (h w) c")

        if cache and (pos_embed := cache.get(shape)) is not None:
            return pos_embed

        pos_embed = rearrange(self.pos_embed, "1 h w c -> 1 c h w")

        cpu_bf16 = pos_embed.device.type == "cpu" and pos_embed.dtype == torch.bfloat16
        if cpu_bf16:
        ...
        if cpu_bf16:
            pos_embed = pos_embed.to(dtype=torch.bfloat16)

        if cache is not None:
            cache[shape] = pos_embed
...
class NaFlexVit(Module):
    ...
    def _forward_blocks(
        self,
        fn: str,
        out: NaFlexFeatures | NaFlexFeaturesVarlen,
        x: Tensor,
        *args,
        intermediates: Collection[int],
        ...
        stop_at = max(idx if idx >= 0 else 27 + idx for idx in intermediates) if intermediates_only else -1
    ...
    def from_srgb(self, seq: Tensor, *, inplace: bool = True) -> Tensor:
        if seq.device != self.embeds.pos_embed.device:
            seq = seq.to(device=self.embeds.pos_embed.device, non_blocking=True)
            inplace = True

        if seq.dtype != self.embeds.pos_embed.dtype:
            seq = seq.to(dtype=self.embeds.pos_embed.dtype)
            inplace = True
